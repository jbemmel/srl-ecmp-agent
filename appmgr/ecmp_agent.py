...
#
# ECMP traceroute agent triggered by any /32 route received through bgp
# It performs an ECMP aware traceroute and updates the FIB with the additional
# information
#
...
## It updates command: info from state ecmp-agent
############################################################
def Update_Result(input_fib, result=True, reason=None, action='add'):
    js_path = '.ecmp_agent.fib_result{.name=="' + input_fib + '"}'
...
##################################################################
## Proc to process the config Notifications received by ecmp_agent
...
def Handle_Notification(obj, file_name, app_id, route_count):
    if obj.HasField('config') and obj.config.key.js_path != ".commit.end":
        ...
        if "ecmp_agent" in obj.config.key.js_path:
...
#                    global pushed_routes
        ...
        if obj.route.data.owner_id == app_id:
            ...
            # JvB NEW -> do ECMP traceroute for /32 IPv4 routes added
            prefix = obj.route.key.ip_prefix.prefix_length
            if prefix==32 and obj.route.op == 0:
                for nh in obj.route.data.nexthop:
                    logging.info(f'Route Nexthop to be processed - {nh}')
                    # for MPLS nexthop v/s IP nexthop
                    if nh.type!=2: # Not MPLS
                        route_count += 1
                        call_traceroute( ipaddress.ip_address(nh.mpls_nexthop.ip_nexthop.addr).__str__() )
                        # TODO process output, associate with route
...
###########################
# JvB: Invokes Paris traceroute command
###########################
def call_traceroute(target_ip):
    logging.info(f'Calling traceroute: ip={target_ip}')
    try:
       tracert_proc = subprocess.Popen(['ip','netns','exec','srbase_default',
                                       '/usr/local/bin/paris-traceroute',
                                       '-amda','-n', target_ip ],
                                       stdout=subprocess.PIPE, stderr=subprocess.PIPE)
       stdoutput, stderroutput = tracert_proc.communicate()
       logging.info(f'call_traceroute result: {stdoutput} err={stderroutput}')
    except Exception as e:
       logging.error(f'Exception caught in paris-traceroute :: {e}')

##################################################################################################
## This is the main proc where all processing for ecmp_agent starts.
## Agent registration, notification registration, Subscrition to notifications.
## Waits on the subscribed Notifications and once any config is received, handles that config
## If there are critical errors, Unregisters the ecmp_agent gracefully.
##################################################################################################
def Run():
...
def Exit_Gracefully(signum, frame):
    logging.info("Caught signal :: {}\n will unregister ecmp_agent".format(signum))
    try:
        response=stub.AgentUnRegister(request=sdk_service_pb2.AgentRegistrationRequest(), metadata=metadata)
